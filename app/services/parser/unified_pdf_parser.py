...
2. 删除目录、纯页码、重复页眉页脚，以及下列能够明确确认无问答价值的污染：
...
4. 允许根据语义进行少量顺序调整，包括调整行顺序、句子结构，根据成句概率推测缺失的信息，但语义必须符合原文，不得随意改变。
5. 只纠正图片能够明确证明的 OCR 错字、异常生僻字、错序和断词。
6. 判断每一项是 paragraph、table_title 还是 table。表名/表题必须单独标为 table_title，不得混入数据行。
7. 如果表格仍以散列文字存在，合并成一个 table 项；其 text 必须是 JSON 对象数组，例如
...
8. 标签必须遵守下游协议：
   ...
   - 如果文档有目录，优先以目录的标题层级为准
   - 不因居中、粗体或字号较大就改变语义标签，标题必须确实是文档结构节点。
   - 仅认定两类内容为标题，一类为有标题序号的，如“一”、“1.”、“（一）”等,二类是没有标号，但明显和下文内容对应的短语，如“引用标准”。
   style 只能是 正文、标题、标题 2、标题 3、标题 4、标题 5、标题 6、表标题、表格。
9. bbox 使用 [x0,y0,x1,y1]，坐标范围统一为 0~1000；无法准确判断时给出近似区域。
10. bold 表示视觉上是否为粗体；font_scale 表示相对正文字号，正文约为 1.0。
